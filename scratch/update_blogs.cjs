...
const path = 'e:/Realtynow_new/src/pages/public/static.tsx';
const content = fs.readFileSync(path, 'utf8');

const replacement = `const DETAILED_BLOGS = [
  {
    id: '1',
...
const headerMatch = content.match(/([\s\S]*?)(?=export function BlogListPage)/);
const footerMatch = content.match(/(export function FaqPage[\s\S]*)/);

if (headerMatch && footerMatch) {
    const newContent = headerMatch[1] + replacement + '\n\n' + footerMatch[1];
