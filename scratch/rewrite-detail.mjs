...
const filePath = 'e:/Realtynow_new/src/pages/public/property-detail.tsx';
let content = fs.readFileSync(filePath, 'utf8');

// The prefix is everything before `{/* Main 2-Column Layout */}` or `<div className="grid gap-8 lg:grid-cols-[1fr_340px]">`
let prefixMatch = content.match(/([\s\S]*?)(\{\/\* Main 2-Column Layout \*\/\}|\{\/\* Property Title & Main Media Showcase \*\/\}|<div className="grid gap-8 lg:grid-cols-\[1fr_340px\]">)/);

let prefix = prefixMatch[1];

let suffixMatch = content.match(/(<section className="mt-16">[\s\S]*)/);
let suffix = suffixMatch ? suffixMatch[1] : '';

const newContent = `      {/* Main 2-Column Layout */}
      <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
        ...
        <div className="space-y-8 min-w-0">
          ...
          <div className="grid gap-2 sm:grid-cols-[1fr_200px]">
            ...
            {/* Thumbnail Grid (2x2) */}
            <div className="hidden sm:grid grid-cols-1 grid-rows-4 gap-2 h-full">
              {images.slice(1, 5).map((img, i) => (
                <button
                  ...
                  className="relative overflow-hidden rounded-xl bg-navy-100 group h-full"
                >
        ...
        <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          ...
          <Card className="p-6 border-t-4 border-t-red-600 shadow-md">
            ...
            <div className="flex items-center gap-4">
              <Avatar name={agentName} src={agent?.avatar_url ?? null} size={56} className="ring-4 ring-red-50" />
...
const finalContent = prefix + newContent + suffix;
fs.writeFileSync(filePath, finalContent);
console.log('Done replacing layout in property-detail.tsx');
